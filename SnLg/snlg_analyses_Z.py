...
@dataclass
class SnLg_OneEventStationPair_Z:
    ...
    z_comp_waveform: Optional[Trace] = None
    z_comp_waveform_for_sn: Optional[Trace] = None
    z_comp_waveform_for_lg: Optional[Trace] = None
...
class SnLg_Analyses_Z:
    ...
    def _process_pair(self, event, stn, network_code, model,
        ...
        self.snlg_onepair = SnLg_OneEventStationPair_Z(event=event._asdict(), station=stn)
    ...
    def _process_pair_synthetic(self, stream, epi_dist_km, model,
        ...
        # Convert the event (pandas namedtuples) to a dict. Pickle may not able to save pandas namedtuples directly.
        if event:
            self.snlg_onepair = SnLg_OneEventStationPair_Z(event=event._asdict(), station=None)
            self.snlg_onepair.evla = evla
            self.snlg_onepair.evlo = evlo
            self.snlg_onepair.mag = mag
            self.snlg_onepair.evdp_err = evdp_err
            ...
            self.snlg_onepair = SnLg_OneEventStationPair_Z(event=None, station=None)
        self.snlg_onepair.pre_filt = pre_filter
        self.snlg_onepair.sn_filt = self.sn_filt
        self.snlg_onepair.lg_filt = self.lg_filt
        self.snlg_onepair.origin_time = origin_time
    ...
    def process_snlg_one_stream(self,stream):
        ...
        Process a single seismic stream (one evnet-station pair) for Sn/Lg analysis:
          ...
          3. Extract the Z component, perform amplitude measurements for Sn, Lg, and noise, and write selected (passes SNR threshold) SAC files.
        """
        ...
        try:
            cor_stream_Z = corrected_stream.select(channel="??Z")
            ###### Further Check: More than 1 traces? ######
            if len(cor_stream_Z) > 1:
                logger.warning(f"{len(cor_stream_Z)} Z traces are found at {self.snlg_onepair.kstnm} for event {self.snlg_onepair.origin_time}")
            ###### Further Check: More than 1 traces? ######
            cor_trace_Z = cor_stream_Z[0]
            if self.enable_archive_waveform:
                self.snlg_onepair.z_comp_waveform = cor_trace_Z.copy() # Archive Z_comp_waveform. Use copy to avoid any further processing.
        except Exception as e:
            logger.error(f"Error in extracting Z component from corrected stream: {e}")
            return
    ...
    def get_SnRLg(self,trace):
        ...
        try:
            z_comp_waveform_for_sn = trace.copy().filter(
                "bandpass",
            ...
            raise
        try:
            z_comp_waveform_for_lg = trace.copy().filter(
                "bandpass",
        ...
        if self.enable_trim_edge:
            ...
            # Be careful with the use of tr.stats.starttime and tr.stats.endtime
            z_comp_waveform_for_sn.trim(z_comp_waveform_for_sn.stats.starttime + self.edge_trim_length/2, 
                                        z_comp_waveform_for_sn.stats.endtime   - self.edge_trim_length/2)
            z_comp_waveform_for_lg.trim(z_comp_waveform_for_lg.stats.starttime + self.edge_trim_length/2, 
                                        z_comp_waveform_for_lg.stats.endtime   - self.edge_trim_length/2)
        
        self.snlg_onepair.z_comp_waveform_for_sn = z_comp_waveform_for_sn.copy() # Use copy to avoid any further processing.
        self.snlg_onepair.z_comp_waveform_for_lg = z_comp_waveform_for_lg.copy() # Use copy to avoid any further processing.
        ...
        try:
            ...
            noise_sn = self.snlg_onepair.z_comp_waveform_for_sn.copy().trim(
                starttime=noise_start,
                endtime=noise_end
            )

            noise_lg = self.snlg_onepair.z_comp_waveform_for_lg.copy().trim(
                starttime=noise_start,
                endtime=noise_end
            )
            ...
            signal_sn = self.snlg_onepair.z_comp_waveform_for_sn.copy().trim(
                starttime = origin_time + sn_start,
                endtime   = origin_time + sn_end
            )

            signal_lg = self.snlg_onepair.z_comp_waveform_for_lg.copy().trim(
        ...
        self.snlg_onepair.SnRLg_raw_ratioed = np.log(
            (self.snlg_onepair.amp_sn / np.max(np.abs(self.snlg_onepair.z_comp_waveform_for_sn.data))) /
            (self.snlg_onepair.amp_lg / np.max(np.abs(self.snlg_onepair.z_comp_waveform_for_lg.data)))
        )

        # Other Sn, Lg Measurements
        # Maximum Amplitude - Is it nessary to apply a normalization by maximum value of the whole trace?
        self.snlg_onepair.max_sn = np.max(np.abs(signal_sn.data)) / np.max(np.abs(self.snlg_onepair.z_comp_waveform_for_sn.data))
        self.snlg_onepair.max_lg = np.max(np.abs(signal_lg.data)) / np.max(np.abs(self.snlg_onepair.z_comp_waveform_for_lg.data))
