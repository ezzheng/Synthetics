...
class SnLg_Analyses:
    def __init__(self, **kwargs):
        ...
        defaults = {
            ...
            "corrected_directory": './Test/corrected/',
            "selected_directory": './Test/selected/',
    ...
    def process_snlg_one_stream(self,stream):
        ...
        logger.info(f"Success rotated to RTZ coordinate system for synthetic event ({self.snlg_onepair.origin_time})")
        ...
        try:
            ...
            logger.info(f"Success in calculating Sn and Lg arrivals for synthetic event ({self.snlg_onepair.origin_time})")
        except Exception as e:
        ...
        # MODIFY TO AXEL METHOD
        # Calculate Sn and Lg windows using group velocity ranges
        # Assume earthquake at Moho for both Sn and Lg windowing
        # Sn group velocity range: 4.0-4.7 km/s
        # Lg group velocity range: 3.0-3.8 km/s

        vsm_calc = self.snlg_onepair.vsm
        vsc_calc = self.snlg_onepair.vsc
        R_calc = self.snlg_onepair.dist
        H_calc = self.snlg_onepair.moho_snlg
        theta_calc = np.arcsin(vsc_calc/vsm_calc)

        l_calc = H_calc/np.cos(theta_calc)
        x_calc = H_calc * np.tan(theta_calc)

        Lg_dist = np.sqrt(R_calc**2 + H_calc**2)
        Sn_dist = R_calc - x_calc + l_calc

        # Calculate Sn window using group velocity range 4.0-4.7 km/s
        # Time = Distance / Velocity
        sn_start = Sn_dist / 4.7  # earliest arrival (fastest velocity)
        sn_end = Sn_dist / 4.0    # latest arrival (slowest velocity)
        
        # Calculate Lg window using group velocity range 3.0-3.8 km/s
        lg_start = Lg_dist / 3.8  # earliest arrival (fastest velocity)
        lg_end = Lg_dist / 3.0    # latest arrival (slowest velocity)
        ...
        if lg_start <= sn_start:
            ...
            logger.info(f"Proceeding to calculate Sn/Lg for synthetic event ({self.snlg_onepair.origin_time})")
    ...
    def get_SnRLg(self,trace):
        ...
        logger.info(f"Success in beginning formal Sn/Lg computation for synthetic event ({self.snlg_onepair.origin_time})")
        ...
        logger.info(f"Success in obtaining Sn and Lg traces synthetic event ({self.snlg_onepair.origin_time})")
